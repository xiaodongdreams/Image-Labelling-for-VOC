...
className = ["smoke","none_of_the_above"]
...
imgs = [(imgs[i], os.path.join(path,imgs[i])) for i in range(len(imgs))]
print(imgs)
for i in range(len(imgs)-1, -1, -1):
    if not imgs[i][1].lower().endswith("jpg") and not imgs[i][1].lower().endswith("png") and not imgs[i][1].lower().endswith("bmp"):
        del imgs[i]
